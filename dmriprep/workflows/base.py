...
def init_dmriprep_wf():
    ...
    for subject_id, session_ids in config.execution.processing_groups:
        log_dir = config.execution.dmriprep_dir / f'sub-{subject_id}'
        ...
        if freesurfer:
            dmriprep_wf.connect(fsdir, 'subjects_dir', single_subject_wf, 'inputnode.subjects_dir')
        else:
...
"""
    ...
    # allow to run with anat-fast-track on dMRI-only dataset
    ...
    workflow.connect([
        ...
        (bidssrc, summary, [('t2w', 't2w'), ('dwi', 'dwi')]),
    ...
    if config.workflow.level == 'full':
        ...
        if freesurfer:
            ...
            ds_fs_segs_wf = init_ds_fs_segs_wf(bids_root=bids_root, output_dir=dmriprep_dir)
            surface_derivatives_wf = init_surface_derivatives_wf()
            ds_surfaces_wf = init_ds_surfaces_wf(output_dir=dmriprep_dir, surfaces=['inflated'])
    ...
    fmap_cache = {}
    if config.execution.derivatives:
        from dmriprep.utils.bids import collect_fieldmaps
    ...
    if all_estimators:
        ...
        if pared_cache:
            ...
            # Note that masks are not emitted. The DWI-fmap transforms cannot be
...
<{', '.join(s.path.name for s in estimator.sources)}>"""
            ...
            if estimator.method == fm.EstimatorType.PEPOLAR:
                ...
                if len(set(suffices)) == 1 or (
                    len(suffices) == 2 and all(suf in ('epi', 'dwi', 'b0') for suf in suffices)
                ):
...
: For each of the {len(dwi_runs)} dMRI runs found per subject (across all
tasks and sessions), the following preprocessing was performed.
"""

    # Before initializing DWI workflow, select/verify anatomical target for coregistration
    if config.workflow.dwi2anat_init in ('auto', 't2w'):
        has_t2w = subject_data['t2w'] or 't2w_preproc' in anatomical_cache
        if config.workflow.dwi2anat_init == 't2w' and not has_t2w:
            raise OSError(
                'A T2w image is expected for DWI-to-anatomical coregistration and was not found'
            )
        config.workflow.dwi2anat_init = 't2w' if has_t2w else 't1w'

    for dwi_series in dwi_runs:
        dwi_file = dwi_series[0]
        fieldmap_id = estimator_map.get(dwi_file)
        jacobian = False
        ...
        dwicache = {}
        if config.execution.derivatives:
            from dmriprep.utils.bids import collect_derivatives, extract_entities

            entities = extract_entities(dwi_series)
        ...
        dwi_wf.__desc__ = dwi_pre_desc + (dwi_wf.__desc__ or '')
        ...
        workflow.connect([
            (buffer, dwi_wf, [('out', f'inputnode.{field}')])
        ...
        if config.workflow.level == 'full':
            ...
            if config.workflow.cifti_output:
                workflow.connect([
                    (select_MNI6_xfm, dwi_wf, [('anat2std_xfm', 'inputnode.anat2mni6_xfm')]),
                    (select_MNI6_tpl, dwi_wf, [('brain_mask', 'inputnode.mni6_mask')]),
                    (anat_fit_wf, dwi_wf, [('outputnode.cortex_mask', 'inputnode.cortex_mask')]),
                    (resample_surfaces_wf, dwi_wf, [
                        ('outputnode.midthickness_fsLR', 'inputnode.midthickness_fsLR'),
                    ]),
                ])  # fmt:skip
...
def map_fieldmap_estimation(
    ...
    # `fmapless` should be set to True to ensure DWI targets are found to be corrected.
    ...
    dwi_files = (dwi_series[0] for dwi_series in dwi_data)

    all_estimators = {
        dwi_file: [fmap_id for fmap_id in get_estimator(layout, dwi_file) if fmap_id in all_ids]
        for dwi_file in dwi_files
    }

    for dwi_file, estimator_key in all_estimators.items():
    ...
    # Final, 1-1 map, dropping uncorrected DWI
    estimator_map = {
        dwi_file: estimator_key[0]
        for dwi_file, estimator_key in all_estimators.items()
        if estimator_key
    }
